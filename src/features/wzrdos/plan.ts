...
export interface WzrdOsPlanTotals {
...
export interface WzrdOsPlanSafety {
...
export interface WzrdOsPlan {
  ...
  totals: WzrdOsPlanTotals;
  warnings: string[];
  safety: WzrdOsPlanSafety;
}
...
function createStep(input: Omit<WzrdOsPlanStep, "id"> & { id: string }): WzrdOsPlanStep {
  return input;
}
...
export function buildWzrdOsPlan(prompt: string, context: WzrdOsPlanContext = {}): WzrdOsPlan {
    ...
    steps.push(
      createStep({
        ...
        input: {
          n: generatedItems,
          brief: command,
          kind: detectContentKind(command),
        },
    ...
    steps.push(
      createStep({
        ...
        input: {
          n: scrapedItems,
          query: command,
          platforms: detectPlatforms(command),
        },
        status: "pending",
        risks: ["read"],
  ...
  if (estimatedCredits > CREDIT_CEILING) {
    warnings.push(`Estimated credits exceed the ${CREDIT_CEILING}-credit ceiling.`);
  }
  if (scheduledPosts > 0) {
    warnings.push("Publishing requires a separate confirmation after scheduling.");
  }
  if (scheduledPosts > 0 && connectedChannels === 0) {
    warnings.push("No connected Postz channels were detected for the schedule step.");
  }
...
export function approveWzrdOsPlan(plan: WzrdOsPlan): WzrdOsPlan {
  return {
    ...plan,
    steps: plan.steps.map((step) => ({
      ...step,
      status: step.requiresApproval ? "approved" : step.status,
    })),
  };
}
...
export function createWzrdOsRunPreview(plan: WzrdOsPlan): WzrdOsRunPreview {
  ...
  plan.steps.forEach((step, index) => {
    const started = new Date(startedAt.getTime() + (index * 2 + 1) * 1000);
    const completed = new Date(startedAt.getTime() + (index * 2 + 2) * 1000);
    events.push({
      id: createId("evt"),
      type: "step.started",
      stepId: step.id,
