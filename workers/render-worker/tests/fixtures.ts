...
export function makeLocalAssets(manifest = makeQCutManifest()): LocalAsset[] {
	...
	return [...paths].map(([path, kind], index) => ({
		...
		probe: {
			...
			videoCodec: kind === "audio" ? null : "h264",
			audioCodec: kind === "visual" ? null : "aac",
			formatName: "fixture",
		},
	}));
}
...
export function makeConfig(overrides: Partial<WorkerConfig> = {}): WorkerConfig {
	return {
		...
		heartbeatMs: 15_000,
		pollMs: 2_000,
		renderTimeoutMs: 45 * 60 * 1_000,
		minFreeDiskBytes: 5 * 1024 * 1024 * 1024,
		retentionSweepMs: 60_000,
		unreferencedOutputRetentionMs: 24 * 60 * 60 * 1_000,
		winningOutputRetentionMs: 14 * 24 * 60 * 60 * 1_000,
		jobRetentionMs: 30 * 24 * 60 * 60 * 1_000,
